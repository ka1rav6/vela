...
import type {
    ...
    IdentExpr, NumberLiteral, BoolLiteral,
    RuleStmt, FactStmt, CondStmt,
} from "./ast.js";
...
describe("Parser", () => {
    ...
    it("parses a RULE with simple ident expr", () => {
        const prog = parser.parse("RULE $canAccess isAdmin");
    ...
    it("multiple statements in one source", () => {
        const src = [
            "FACT $admin true",
            "FACT $age 25",
            "RULE $canAccess admin AND (age >= 18)",
        ].join("\n");
...
describe("Parser Errors", () => {
    ...
    it("throws on RULE without dollar", () => {
        assert.throws(() => parser.parse("RULE x y"), ParseError);
    });
